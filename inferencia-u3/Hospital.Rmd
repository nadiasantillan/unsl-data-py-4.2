...
d <- cohens_d(antes_pilot, desp_pilot);d
...
# ------------------------------------------------------------------------------
d_todas_salas <- cohens_d(tiempo_espera ~ protocolo, data = datos);d_todas_salas
g_todas_salas <- hedges_g(tiempo_espera ~ protocolo, data = datos);g_todas_salas
glass_todas_salas <- glass_delta(tiempo_espera ~ protocolo, data = datos);glass_todas_salas
...
cat(sprintf("χ² = %.2f, p = %.3f\n", chs_t$statistic, chs_t$p.value))
# Intervalo de confianza
m <- matrix(c(70, 130, 310, 430), nrow=2, byrow=T)
oddsratio(m)
cat(sprintf("Odds ratio %.2f IC 95%% [%.2f, %.2f] \n", or$Odds_ratio, or$CI_low, or$CI_high))
cat(sprintf("Disminución %.2f IC 95%% [%.2f, %.2f] %%\n", 1-or$Odds_ratio, 1-or$CI_high, 1-or$CI_low))
```
```{r}
# -------------------------------------------------------------
...
cat(sprintf("η² = %.3f IC 95%% [%.3f, %.3f]\n", eta2$Eta2, eta2$CI_low, eta2$CI_high))
cat(sprintf("ω² = %.3f IC 95%% [%.3f, %.3f]\n", omega2$Omega2, omega2$CI_low, omega2$CI_high))
cat(sprintf("ε² = %.3f IC 95%% [%.3f, %.3f]\n", epsilon2$Epsilon2, epsilon2$CI_low, epsilon2$CI_high))
```
